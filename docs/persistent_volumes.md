...
An example of using this dynamic provisioning is to create and use the following volume claim.

```yaml
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
    name: persistent-host-files
spec:
    accessModes:
        - ReadWriteMany
    resources:
...
Your host will then have a subdirectories with different pvc's for each pod that uses this under /tmp/hostpath-provisioner.
